...
def CT(b):
    ...
    return np.pad(
        b, ((v_pad, v_pad), (h_pad, h_pad)), "constant", constant_values=(0, 0)
    )
...
def precompute_X_divmat():
    ...
    return 1.0 / (CT(np.ones(sensor_size)) + mu1)
...
def V_update(w, rho, u, eta, x, xi, H_fft, R_divmat):
    freq_space_result = R_divmat * fft.fft2(
        fft.ifftshift(r_calc(w, rho, u, eta, x, xi, H_fft))
    )
...
def runADMM(psf, data):
    ...
    for i in range(iters):
        X, U, V, W, xi, eta, rho = ADMM_Step(
            X, U, V, W, xi, eta, rho, [H_fft, data, X_divmat, R_divmat]
        )
        if i % disp_pic == 0:
            print(i)
            image = C(V)
